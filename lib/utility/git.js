...
Module.init = function (path, domain) {
	...
	fse.copyAsync(gitignorePath, gitignoreTargetPath)
		...
		.then(function () {
			Logging.info(util.format('Pushing %s to remote (%s)', Module.masterBranchName, Module.remoteName));
		...
		.then(function () {
			Logging.info(util.format('Creating %s branch on the Git repository', Module.developBranchName));
		...
		.then(function () {
			Logging.info(util.format('Checking out %s branch from the Git repository', Module.developBranchName));
		...
		.then(function () {
			Logging.info(util.format('Performing initial commit to %s branch in the Git repository', Module.developBranchName));
		...
		.then(function () {
			Logging.info(util.format('Pushing %s to remote (%s)', Module.developBranchName, Module.remoteName));
