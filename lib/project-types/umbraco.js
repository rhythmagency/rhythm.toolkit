...
module.exports = function (name, path) {
	var resolver = Promise.pending(),
		templatePath = nodePath.resolve(__dirname, '../../templates/umbraco'),
	...
	fse.copy(templatePath, path, function (err) {
		if (err) {
			...
			var extractUmbraco = function (zipFile, websitePath) {
				/*
				...
				setTimeout((function () {
					...
					// TODO: Make sure we add .gitkeep files to all empty directories
			...
			if(fs.existsSync(umbracoZipPath)) {
				extractUmbraco(umbracoZipPath, websitePath);
			} else {
				// Download umbraco
